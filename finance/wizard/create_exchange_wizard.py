...
class CreateExchangeWizard(models.TransientModel):
    ...
    def _compute_period_id(self):
        for ex in self:
            ex.period_id = self.env['finance.period'].get_period(ex.date)

    @api.model
    def _get_last_date(self):
        return self.env['finance.period'].get_period_month_date_range(
            self.env['finance.period'].get_date_now_period_id())[1]
    ...
    def create_partner_exchange_line(self, vals):
        ...
        for line in voucher_lines:
            ...
            if line.debit == 0:
                val.update({
                    'currency': val.get('currency') + line.currency_amount,
        ...
        if account.balance_directions == 'in':
            ...
            if currency_amount * vals.get('rate_silent') - debit + credit != 0:
                self.env['voucher.line'].create({
                    'name': "汇兑损益",
                    'account_id': account_id,
                    'debit': currency_amount * vals.get(
                        'rate_silent') - debit + credit,
                    'voucher_id': vals.get('vouch_obj_id'),
                    'currency_id': False,
                    'currency_amount': False,
                    'rate_silent': False,
            ...
            if currency_amount * vals.get('rate_silent') - credit + debit != 0:
                self.env['voucher.line'].create({
                    'name': "汇兑损益",
                    'account_id': account_id,
                    'credit': currency_amount * vals.get(
                        'rate_silent') - credit + debit,
                    'voucher_id': vals.get('vouch_obj_id'),
                    'currency_id': False,
                    'currency_amount': False,
                    'rate_silent': False,
