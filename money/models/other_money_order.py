...
class OtherMoneyOrder(models.Model):
        ...
        if self.env.context.get('type') == 'other_get':
            values.update(
                {'name': self.env['ir.sequence'].next_by_code(
                    'other.get.order')})
        if self.env.context.get('type') == 'other_pay' \
                or values.get('name', '/') == '/':
            values.update(
                {'name': self.env['ir.sequence'].next_by_code(
                    'other.pay.order')})
    ...
    def _compute_details(self):
        for v in self:
            ...
            for li in v.line_ids:
                vl['val'].append([
                    li.category_id.name,
                    li.account_id.display_name,
                    li.auxiliary_id.name or '',
                    li.amount])
    ...
    def other_get_create_voucher_line(self, vouch_obj, init_obj):
        ...
        for line in self.line_ids:
            ...
            vals.update({'vouch_obj_id': vouch_obj.id,
                         'name': self.name, 'note': line.note or '',
                         ...
                         'currency_amount': (
                            not in_is_company_currency
                            and self.total_amount or 0),
                         # 判断是否本币
            ...
            if vals.get('sell_tax_amount'):
                ...
                self.env['voucher.line'].create({
                    ...
                    'credit': in_is_company_currency
                        and line.tax_amount
                        or line.tax_amount * vals.get('rate_silent') or 0,
                    'voucher_id': vals.get('vouch_obj_id'),
                    'currency_amount':
                        not in_is_company_currency
                        and line.tax_amount or 0,
                    'rate_silent': vals.get('rate_silent'),
        ...
        self.env['voucher.line'].create({
            ...
            'debit':
                ...
                or self.total_amount * vals.get('rate_silent'),
            'voucher_id': vals.get('vouch_obj_id'),
    ...
    def other_pay_create_voucher_line(self, vouch_obj):
        ...
        for line in self.line_ids:
            ...
            rate_silent = self.env['res.currency'].get_rate_silent(
                self.date, self.bank_id.currency_id.id)
            vals.update({'vouch_obj_id': vouch_obj.id, 'name': self.name,
                         'note': line.note or '',
                         ...
                         'currency_id': self.bank_id.currency_id.id,
                         'currency_amount':
                             not out_is_company_currency
                             and self.total_amount or 0,  # 判断是否本币
                         'rate_silent': rate_silent,
                         })
            # 借方行
            self.env['voucher.line'].create({
