...
class TestVoucher(TransactionCase):
    ...
    def test_voucher_can_be_draft(self):
        ...
        voucher = self.env.ref('finance.voucher_1')
        voucher.voucher_done()
        voucher.voucher_can_be_draft()    #是否应该放在上面的case里
    ...
    def test_voucher_done_costs_types_in(self):
        '''收入类科目只能在贷方记账'''
        voucher = self.env['voucher'].with_context(
            {'entry_manual': 1}).create({
                'date': '2017-01-01',
                'line_ids': [
                    (0, 0, {
                        'name': '退款给客户',  # 贷方行
    ...
    def test_restricted_account(self):
        ...
        self.env.ref('finance.voucher_1').with_context(
            {'entry_manual': 1}).write({'att_count': '15'})
    ...
    def test_default_voucher_date_last(self):
        ...
        self.env['ir.default'].set(
            'finance.config.settings', 'defaul_voucher_date', 'last')
#        voucher_obj._default_voucher_date()
        voucher_obj.create({})
...
class TestVoucherLine(TransactionCase):
    ...
    def test_post_view(self):
        ''' 只能往下级科目记账 '''
        with self.assertRaises(UserError):
            self.env.ref('finance.voucher_line_1_debit'
                         ).account_id = self.env.ref(
                'finance.account_chart1002')
...
class TestFinanceAccount(TransactionCase):
    ...
    #         'name': u'测试科目显示出余额',
    #         'account_id': self.cash.with_context(
    #               {'show_balance': True}).id,    # 给该字段传context没有成功
    #     })
    #     name = self.cash.name_get()
    #     real_name = '%s %s %s' % (
    #           self.cash.code, self.cash.name, self.cash.balance)
    #     self.assertTrue(name[0][1] == real_name)
    ...
    def test_unlink(self):
        ...
        self.env.ref('finance.small_business_chart2221001').child_ids.unlink()
    ...
    def test_write(self):
        ...
        with self.assertRaises(UserError):
            ...
            self.cash.with_context(
                {'modify_from_webclient': 1}).write({'source': 'init'})
        # 界面上记过账科目不能修改
        account_bank = self.env.ref('finance.account_bank')
        with self.assertRaises(UserError):
            account_bank.source = ''
            account_bank.with_context(
                {'modify_from_webclient': 1}).write({'source': 'init'})
    ...
    def test_add_child(self):
        ...
        # 非末级科目增加子科目
        wizard = self.env['wizard.account.add.child'].with_context(
            {'active_id': self.env.ref(
                'finance.small_business_chart1701').id}).create({
                    'account_code': '08',
                    'account_name': '',
                })
        wizard.create_account()
        ...
        self.env['ir.default'].set(
            'finance.config.settings', 'defaul_account_hierarchy_level', '2')
        business_chart170108 = self.env['finance.account'].search(
            [('code', '=', '170108')])
        with self.assertRaises(UserError):
            wizard = self.env['wizard.account.add.child'].with_context(
                {'active_id': business_chart170108.id}). \
                create({
                    'account_code': '01',
                    'account_name': 'test'
                })
...
class TestVoucherTemplateWizard(TransactionCase):
    def setUp(self):
        ...
        self.voucher_template_wizard = self.env[
            'voucher.template.wizard'].create({
...
class TestVoucherTemplateLine(TransactionCase):
    ...
    def test_onchange_account_id(self):
        ...
        for line in template.line_ids:
            ...
            line.account_id = self.env.ref(
                'finance.small_business_chart1403').id
            line.onchange_account_id()
            line.account_id.auxiliary_financing = 'goods'
            line.onchange_account_id()
            line.account_id.auxiliary_financing = 'customer'
...
class TestCheckoutWizard(TransactionCase):
    ...
    def test_recreate_voucher_name(self):
        ...
        # 按月 重排结账会计期间凭证号
        setting_row_month = self.env['finance.config.settings'].create(
            {"defaul_period_domain": "can",
             "defaul_reset_init_number": 1,
             "defaul_auto_reset": True,
             "defaul_voucher_date": "today"})
        ...
        # 按年 重排结账会计期间凭证号
        setting_row_year = self.env['finance.config.settings'].create({
            "defaul_period_domain": "can",
            "defaul_reset_init_number": 1,
            "defaul_auto_reset": True,
        ...
        # 按年重置 上一个期间不存在
        period_id_init = self.env['finance.period'].search(
            [], order='year, month', limit=1)
        period_id_init.is_closed = True
    ...
    def test_get_last_date(self):
        ...
        day = calendar.monthrange(
            int(datetime_str_list[0]), int(datetime_str_list[1]))

        c_w = self.env['checkout.wizard'].create({})
        self.assertEqual(c_w.date.strftime("%Y-%m-%d"),
                         '-'.join(
            [datetime_str_list[0], datetime_str_list[1], str(day[1])]))
