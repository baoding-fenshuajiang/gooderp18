...
class Partner(models.Model):
    ...
    def _set_payable_init(self):
        ...
        if self.payable_init:
            ...
            self._init_source_create(
                "期初应付余额", self.id, categ.id, True,
                self.env.user.company_id.start_date,
                self.s_category_id.account_id.currency_id.id,
                self.payable_init, 0,
                self.payable_init,
                self.env.user.company_id.start_date, 'draft')
    ...
    invoice_ids = fields.One2many(
        'money.invoice', 'partner_id', '未清结算单',
        domain=[
            ('to_reconcile', '>', 0),
            ('state', '=', 'done'),
            ('date_due', '<=', datetime.today().date())])
    money_ids = fields.One2many(
        'money.order', 'partner_id', '未清付款',
    ...
    def partner_statements(self):
        ...
        if (self.c_category_id.type == 'customer'
                and self.env.context.get('is_customer_view')):
...
class BankAccount(models.Model):
    ...
    def _set_init_balance(self):
        ...
        start_date_period_id = self.env['finance.period'].search_period(
            start_date)
        ...
        if self.init_balance:
            # 资金期初 生成 其他收入
            money_get = self.with_context(
                type='other_get').env['other.money.order']
            other_money_init = money_get.create({
                'bank_id': self.id,
